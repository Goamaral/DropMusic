/**
 * The objective of this project is to create a system to manage and share music whith features similar to AllMusic.com and IMDb.com
 * We decided to go with a structure closer to the MVC architecture for a better readability and to be easier to add new features, and changes things in the future.
 * The architeture is built by RMI Server, RMI Client, and Multicast Server. The Client is the front end of the application where the user can navigate through menus and make actions with them.
 * The comunication between RMI Client and RMI Server uses RMI (Remote Method Invocation) protocol. In addiction we also made an protocol to the RMI Server to exchange messages with Multi cast Server(DataStorage).
 * These messages are being sent through Multicast protocol. The messages cointain relevant changes to be made to accomplish what the user wants to be done. The actions that the user can make are CRUD based.
 * The protocol made to comunicate between the multicast server and the RMI Server consists in the important object serialization the required action, after this the multicast serve will deserialize the string and convert it to the desired object. The multicast server will then execute the action the user required and make the response, serializing it to send it back to the RMI Server and Client.
 * The RMI Server will recieve an action from the client and will handle it accordingly with the above description, but not without making the possible validations as check if the form for a certain action is empty.
 * The Core contains all the dependencies that are needed across the project.
 *
 * The shores were divided according to the assignment, one member do the Client Side, inclusive the RMI Server and the other the muticast part (protocol)
 *
 * -@Requisitos Funcionais
 * -Registar novo utilizador --> Pass
 * -Login protegido com password --> Pass
 * -Introduzir artistas, álbuns e músicas --> Pass
 * -Pesquisar álbuns por artista e por título de álbum --> Pass
 * -Consultar detalhes de álbum (incluindo músicas e críticas) --> Pass
 * -Editar detalhes de álbum (incluindo músicas) --> Pass
 * -Escrever crítica sobre um álbum (com pontuação) --> Pass
 * -Consultar detalhes de artista (e.g., discografia, biografia) --> Pass
 * -Dar privilégios de editor a um utilizador --> Pass
 * -Notificação imediata de privilégios de editor (online users) --> Pass
 * -Notificação imediata de re-edição de descrição de álbum (online users) --> Pass
 * -Entrega posterior de notificações (offline users) --> Pass
 * -Upload de ficheiro para associar a uma música existente --> Pass
 * -Partilhar um ficheiro musical e permitir o respetivo download --> Pass
 *
 * @Tratamento de Exceções
 * -Avaria de um servidor RMI não tem qualquer efeito nos clientes --> Pass
 * -Não se perde/duplica músicas se os servidores RMI falharem --> Pass
 * -O serviço funciona desde que haja um servidor multicast disponível --> Pass
 * -Avarias temporárias (<30s) dos N servidores são invisíveis para clientes --> Pass
 * -Pedidos são sempre processados por N>=1 servidores multicast --> Pass
 * -Pedidos de leitura são respondidos apenas por um servidor multicast --> Fail
 *
 * @Failover
 * -Servidor RMI primário testa periodicamente o secundário --> Pass
 * -Em caso de avaria longa os clientes RMI ligam ao secundário --> Pass
 * -Servidor RMI secundário substitui o primário em caso de avaria longa --> Pass
 * -O failover é invisível para utilizadores (não perdem a sessão) --> Pass
 * -O servidor original, quando recupera, torna-se secundárioRequisitos Funcionais --> Pass
 *
 *
 * @author  Henrique Ferrer
